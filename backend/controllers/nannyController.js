...
export const getAllNannies = catchAsync(async (req, res) => {
  const { page = 1, perPage = 3, sort = 'a_to_z', filter = 'show_all' } = req.query;
  ...
  const filterQuery = filterEntry.query;
  const sortOrder = filterEntry.defaultSort && sort === 'a_to_z'
...
export const getNannyById = catchAsync(async (req, res ) => {
  ...
  res.status(200).json({ data: nanny });
});